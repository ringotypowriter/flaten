...
- Optional plain-text transcript output via `--format txt` (one snippet per line; separator configurable via `--separator`, defaults to newline; `--txt` is kept as a shorthand alias).
